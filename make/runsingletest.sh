...
if isPython $SUITE; then
	...
	$PYTHON_EXECUTABLE $VOODOO_ROOT_DIR/pytest/pytestrunner.py --verbose --singleTest=$TEST_NAME $SUITE
else
    if [ ! isCxxTest ]; then
        echo "$SUITE was not recognized as niether python test suite or CxxTest suite files"
        exit 1
    fi
    TEST_NAME=`$PYTHON_EXECUTABLE $VOODOO_ROOT_DIR/pytest/findtestname.py $SUITE $REGEX_OR_LINE_NUMBER`
